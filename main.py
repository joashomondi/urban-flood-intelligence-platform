...
def _ensure_engine() -> None:
    """Bootstrap ``src`` in the Zerve deployment sandbox (no git needed)."""
    try:
        import src  # noqa: F401
        return
    except ModuleNotFoundError:
        pass
    cache = Path("/tmp/ufip_engine") if Path("/tmp").exists() \
        else Path(tempfile.gettempdir()) / "ufip_engine"
...
try:
    with st.spinner("Loading flood intelligence state…"):
        STATE = load_state()
    SCORER = STATE["scorer"]
except Exception as exc:
    st.error("Dashboard failed to start.")
    st.code(str(exc))
    st.info(
        "If this deployment is linked to your canvas, re-run **Run All** on the "
        "six blocks first, then **Update** this deployment. "
